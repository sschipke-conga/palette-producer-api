...
describe('Server', () => {
  ...
  describe('POST /api/v1/signup', () => {
    ...
    it('should return a 422 and a message stating the missing parameter', async () => {
      ...
      expect(response.body.error).toEqual('Expected format: {username: <string>, password <string>. You\'re missing a \'username\' property.');
    });
  });
  ...
  describe('GET /api/v1/users/:user_id/projects', () => {
    ...
    it('should return a 404 and a message that there are no projects', async () => {
      ...
      // execution
      const response = await request(app).get(`/api/v1/users/${invalidId}/projects`);
  ...
  describe('GET /api/v1/projects/:project_id/palettes', () => {
    it('should return a 200 and all palettes for a specific user\'s project', async () => {
      ...
      // execution
      const response = await request(app).get(`/api/v1/projects/${project_id}/palettes`);
  ...
  describe('POST /api/v1/projects', () => {
    ...
    it('should return a 422 if a new project\'s information is unprocessable', async () => {
      ...
      // expectation
      expect(response.status).toBe(422);
      expect(response.body.error).toEqual('Expected format: { user_id: <integer>, name: <string>}. You\'re missing a \'user_id\' property.');
    });
  });
  ...
  describe('POST /api/v1/palettes', () => {
    it('should post a new palette to the db', async () => {
      ...
      const { id } = project;
      const newPalette = { name: 'Green', color1: '#asdfgh', color2: '#qwerty', color3: '#ghjklp', color4: '#123456', color5: '#098765', project_id: id };
  ...
  describe('PATCH /api/v1/projects/:id', () => {
    it('should patch a project in the db', async () => {
      ...
      // execution
      const response = await request(app).patch(`/api/v1/projects/${id}`).send(newProjectPatch);
    ...
    it('should return a 404 if project does not exist with attempted id', async () => {
      ...
      // execution
      const response = await request(app).patch(`/api/v1/projects/${invalidID}`).send(newProjectPatch);

      // expectation
      expect(response.status).toBe(404);
      expect(response.body.error).toEqual(`No existing project with id of ${invalidID}`);
    });
    it('should return a 422 if project patch attempts to change a parameter that does not exist', async () => {
      ...
      // expectation
      expect(response.status).toBe(422);
      expect(response.body.error).toEqual(`You can only update a projects's <name> not hillmoprolly`);
    });
  });
  ...
  describe('DELETE /api/v1/projects/:id', () => {
    ...
    it('should return a 404 if project id does not exist', async () => {
      ...
      // expectation
      expect(response.status).toBe(404);
      expect(response.body).toEqual(`Project with an id: ${invalidID} does not exist.`);
    });
  });
  ...
  describe('DELETE /api/v1/palettes/:id', () => {
    ...
    it('should return a 404 if palette id does not exist', async () => {
      ...
      // expectation
      expect(response.status).toBe(404);
      expect(response.body).toEqual(`Palette with an id: ${invalidID} does not exist.`);
    });
  });
});
